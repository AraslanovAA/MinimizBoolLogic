...
class Truths(object):
    def __init__(self, base=None, phrases=None, ints=True):
        ...
        self.base = base#это переменные
        self.phrases = phrases or []#это выражения
        self.ints = ints

        # generate the sets of booleans for the bases
        self.base_conditions = list(itertools.product([False, True],#формирует заранее известную часть таблицы в виде списка из ()
                                                      repeat=len(base)))
    ...
    def calculate(self, *args):#args это () из base_condition
        # store bases in an object context
        g = Gob()
        for a, b in zip(self.base, args):
            setattr(g, a, b)#типо по полю а поле б?
